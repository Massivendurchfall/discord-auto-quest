(async () => {
    ...
    const Storage = {
        save(key, value) {
            try { window.localStorage.setItem(`tape_${key}`, JSON.stringify(value)); }
            catch (e) { console.debug('[Storage] Write failed:', e.message); }
        },
        load(key) {
            try { const v = window.localStorage.getItem(`tape_${key}`); return v ? JSON.parse(v) : null; }
            catch (e) { return null; }
        }
    };
    ...
    const Logger = {
        ...
        init() {
            const oldUI = document.getElementById('tape-ui'); if (oldUI) oldUI.remove();
            const oldStyle = document.getElementById('tape-styles'); if (oldStyle) oldStyle.remove();
            ...
            style.innerHTML = `
                ...
                #tape-head { padding: 14px 16px; background: #1e1f22; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #2b2d31; cursor: grab; user-select: none; }
                #tape-head:active { cursor: grabbing; background: #232428; }
                #tape-title { font-weight: 800; font-size: 14px; color: #fff; display: flex; align-items: center; gap: 8px; letter-spacing: 0.5px; }
                #tape-title svg { color: ${CONFIG.THEME}; }
                #tape-controls { display: flex; gap: 10px; align-items: center; }
                ...
                #tape-body { padding: 12px 8px 12px 12px; max-height: 450px; overflow-y: auto; flex-grow: 1; scrollbar-gutter: stable; display: flex; flex-direction: column; }
                #tape-body.picker-mode { overflow: hidden; }
                #tape-picker-form { display: flex; flex-direction: column; flex: 1 1 auto; min-height: 0; }
                #tape-ui ::-webkit-scrollbar { width: 4px; height: 4px; }
                #tape-ui ::-webkit-scrollbar-track { background: none; }
                #tape-ui ::-webkit-scrollbar-thumb { background: #5e5f69; border-radius: 4px; }
                #tape-ui ::-webkit-scrollbar-thumb:hover { background: #2b2d31; }
                ...
                #tape-logs { padding: 10px 12px; background: #0e0f10; font-family: 'Consolas', 'Monaco', monospace; font-size: 11px; color: #949ba4; height: 140px; overflow-y: auto; border-top: 1px solid #2b2d31; scroll-behavior: smooth; }
                ...
                #tape-footer { padding: 8px; text-align: center; background: #191b1e; border-top: 1px solid #2b2d31; font-size: 10px; color: #72767d; }
                ...
                .tape-option { display: flex; justify-content: space-between; align-items: center; background: #1e1f22; padding: 10px 12px; border-radius: 6px; border: 1px solid #2b2d31; }
                .tape-option-label { font-size: 12px; font-weight: 600; color: #dbdee1; }
                .tape-toggle { position: relative; width: 32px; height: 18px; flex-shrink: 0; }
                .tape-toggle input { opacity: 0; width: 0; height: 0; position: absolute; }
                ...
                .tape-toggle input:checked + .slider { background-color: ${CONFIG.SUCCESS}; }
                .tape-toggle input:checked + .slider:before { transform: translateX(14px); }
            ...
            this.root.id = 'tape-ui';
            this.root.innerHTML = `
                <div id="tape-head">
                    <span id="tape-title">${ICONS.BOLT} ${CONFIG.NAME} <span style="opacity:0.5; font-size:10px; margin:2px 0 0 5px;">${CONFIG.VERSION}</span></span>
                    <div id="tape-controls">
                        <span class="ctrl-btn ctrl-stop" id="tape-stop" title="Stop script & cleanup">${ICONS.STOP} STOP</span>
                        <span class="ctrl-btn" style="font-size:10px; color:#949ba4;" id="tape-close" title="Shift + .">HIDE</span>
                    </div>
                </div>
            ...
            document.getElementById('tape-body').addEventListener('click', async (e) => {
            ...
            document.getElementById('tape-close').onclick = () => this.toggle();
            document.getElementById('tape-stop').onclick = () => this.shutdown();
        ...
        shutdown() {
            ...
            Patcher.clean();
            setTimeout(() => {
                const styles = document.getElementById('tape-styles');
                if (styles) styles.remove();
                if (this.root?.parentElement) this.root.remove();
                window.tapeLock = false;
            }, 1000);
        },
        ...
        updateTask(id, data) {
            ...
            if (oldData && oldData.status === newData.status && oldData.removing === newData.removing &&
                ...
                const card = document.getElementById(`tape-task-${id}`);
                if (card) {
        ...
        log(msg, type = 'info') {
            ...
            console.log(`%c[TAPE] %c${msg}`, `color: ${CONFIG.THEME}; font-weight: bold;`, `color: ${colors[type] || colors.info}`);
            try {
                const box = document.getElementById('tape-logs');
                if (box && type !== 'debug') {
        ...
        render() {
            if (document.getElementById('tape-picker-form')) return;
        ...
        showQuestPicker(quests) {
            return new Promise((resolve) => {
                const body = document.getElementById('tape-body');
                const logs = document.getElementById('tape-logs');
                ...
                const buildToggle = (name, label, isChecked) => `
                    <div class="tape-option">
                        <span class="tape-option-label">${label}</span>
                ...
                body.innerHTML = `
                    <form id="tape-picker-form">
                        ...
                        <div id="tape-quest-list" class="picker-quest-list">
                ...
                const form = document.getElementById('tape-picker-form');
        ...
        async finish(q, t) {
            ...
            try {
                if (typeof Notification !== 'undefined' && Notification.permission === "granted") {
                    new Notification("Tape: Quest Completed", { body: t.name, icon: "https://cdn.discordapp.com/emojis/1120042457007792168.webp", tag: `tape-${q.id}` });
                }
    ...
    main().catch(e => {
        ...
        console.error('[Tape Fatal]', e);
        try { Logger.log(`[System] FATAL: ${msg}`, 'err'); } catch (_) { }
        Logger.shutdown();

        setTimeout(() => { window.tapeLock = false; }, 1500);
    });
})();
