...
if(quests.length === 0) {
	...
	let doJob = function() {
		...
		if(taskName === "WATCH_VIDEO" || taskName === "WATCH_VIDEO_ON_MOBILE") {
			...
			if(!isApp) {
				console.log(`Non-video quests no longer work in browser. Use desktop app for ${questName} quest!`);
			} else {
				api.get({url: `/applications/public?application_ids=${applicationId}`}).then(res => {
					...
					let fn = data => {
						...
						console.log(`Progress: ${progress}/${secondsNeeded}s`);
					...
					console.log(`Spoofed game: ${applicationName}. Wait approx. ${Math.ceil((secondsNeeded - secondsDone) / 60)} minutes.`);
				});
			}
			...
			if(!isApp) {
				...
				FluxDispatcher.subscribe("QUESTS_SEND_HEARTBEAT_SUCCESS", fn);
				console.log(`Spoofed stream: ${applicationName}. Wait approx. ${Math.ceil((secondsNeeded - secondsDone) / 60)} minutes.`);
			}
